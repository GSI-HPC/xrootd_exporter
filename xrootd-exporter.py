...
class xrootd_exporter:
    ...
    def fetch_mpxstat(self):
        ...
        def read_line(): return self.mpx.stdout.readline().decode('utf-8').rstrip()
        line = read_line()
        while line != "":
            k, v = line.split(' ')
            self.mpx_stats[k] = v
    ...
    def verify_mpx_running(self):
        ...
        if rc != None:
            raise Exception(f"mpxstats process has terminated with {rc}")

    def __init__(self, mpx_port=10024, mpx_path='/usr/bin/mpxstats'):
        # create mpxstats process
        self.mpx = Popen([mpx_path, '-f', 'flat', '-p',
                         str(mpx_port)], stdout=PIPE)
        self.mpx_stats = {}
        self.fetch_mpxstat()

        # helper lambdas
        # replace dots which are not allowed in prometheus vars
        def repl_dots(st): return st.replace(".", "_")
        def run_cmd(cmd, stdout=PIPE): return str(
            run(cmd, stdout=stdout).stdout, 'utf-8').rstrip()

        def get_mpxstat(key): return lambda: self.mpx_stats[key]
        self.updt_mpx_infos = lambda: self.mpx_infos.info(
            {k: v for k, v in self.mpx_stats.items() if k in info_keys})

        # filterlist for non float info keys
        info_keys = ["ver", "src", "pgm",
                     'oss.paths.0.lp', 'oss.paths.0.rp', 'ofs.role']
